...
# Publish to Artifactory (requires credentials in gradle.properties)
./gradlew publish
...
Traditional CRDT libraries embed versions within values (e.g., `LWWRegister<String>`), creating O(F × D) overhead for F fields at depth D. This library achieves O(m) overhead where m = modified field count.
...
- Tested in production restaurant environments with network partitions
