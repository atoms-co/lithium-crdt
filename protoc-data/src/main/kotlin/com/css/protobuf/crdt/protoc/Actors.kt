...
fun Actors?.incrementLocalActor(actorGenerator: () -> Long = Random::nextLong): Actors {
    ...
    val localActor = localActor.takeIf { it != 0L } ?: actorGenerator()
    return toBuilder()
        .setLocalActor(localActor)
        .putAllVersionVector(versionVectorMap.increment(localActor))
        .build()
}
 ...
 * ```
 */
fun Actors?.merge(incoming: Map<Long, Long>) = this?.run {
    val local: Map<Long, Long> = versionVectorMap
    toBuilder().putAllVersionVector(local.merge(other = incoming, combine = ::maxOf)).build()
} ?: Actors.newBuilder().putAllVersionVector(incoming).build()
